...
#         return -corr
        ...
        New implementation based on sklearn's CCA implementation

        """  
        tensor1 = H1.clone().detach()
        H1 = tensor1.detach().numpy()       
        tensor2 = H2.clone().detach()
        H2 = tensor2.detach().numpy()       
        n_comps = self.outdim_size//2
        cca = CCA(n_components=n_comps,  max_iter=1000)
        cca.fit(H1, H2)
        corr = cca.score(H1,H2)
        l = 1 - corr
